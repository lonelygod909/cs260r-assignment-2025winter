...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        return (
            torch.FloatTensor(self.state[ind]).to(self.device), torch.FloatTensor(self.action[ind]).to(self.device),
            torch.FloatTensor(self.next_state[ind]).to(self.device),
            torch.FloatTensor(self.reward[ind]).to(self.device), torch.FloatTensor(self.not_done[ind]).to(self.device)
        )
...
class TD3Trainer:
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        with torch.no_grad():
            ...
            # Hint: Sample a noise from Normal distribution with the STD=self.policy_noise.
            # The shape should be the same as the action.
            noise = (torch.normal(mean=0, std=self.policy_noise, size=action.shape, device=action.device)
            ...
            # The shape of next_action should be the same as the action.
            # next_action = (
            #         ???
            # ).clamp(-self.max_action, self.max_action)
            next_action = (self.actor_target(next_state).reshape(action.shape) + noise
                           ).clamp(-self.max_action, self.max_action)
            ...
            target_Q = reward + self.discount * not_done * torch.min(target_Q1, target_Q2)
