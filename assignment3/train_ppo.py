...
parser.add_argument("--num-envs", default=10, type=int, help="The number of parallel environments. Default: 10")
parser.add_argument("--seed", default=0, type=int, help="The random seed. Default: 0")
parser.add_argument("--max-steps", "-N", default=1e7, type=float, help="The random seed. Default: 1e7")
parser.add_argument(
    "--num-steps",
    default=2000,
    type=int,
...
parser.add_argument("--lr", default=-1, type=float, help="Learning rate. Default: 5e-5")
parser.add_argument("--gae-lambda", default=0.95, type=float, help="GAE coefficient. Default: 0.95")
parser.add_argument(
    "--num-epoch",
    default=10,
    type=int,
...
parser.add_argument(
    "--restore", action="store_true", help="Whether to load checkpoint-final and continue learning. Default: False"
)
    ...
    with tqdm.tqdm(total=int(args.max_steps)) as pbar:
        while True:
            ...
            if iteration % config.log_freq == 0:
                ...
                pretty_print({"===== {} Training Iteration {} =====".format(algo, iteration): stats})
            ...
            if iteration % config.save_freq == 0:
                ...
                print("Trainer is saved at <{}>. Progress is saved at <{}>.".format(trainer_path, progress_path))
